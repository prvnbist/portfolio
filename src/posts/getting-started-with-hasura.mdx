...
tags: ['hasura', 'getting started', 'postgres', 'sql', 'graphql']
---
...
Give your app a name and select the you want to deploy to and then click on `Deploy App` button and wait for Heroku to deploy your app.
...
-  **Events:** Events comes in when you want execute custom logic based on event triggers such on creating, updating or deleting a row in a table.
...
Now hit `Add Table` button to add the table. Now if you go to the `GraphiQL` tab you'll see that Hasura has generated queries for you which you start exploring.
...
The name of the queries are also generated based on table name, which can be changed from:

> Data > Select Table > Modify > Custom GraphQL Root Fields.

After modifying, just save your table and Hasura will update the query names for you.
...
-  `where`: Use when needed to find an rows/objects based on a comparison done of different fields. Hasura is smart enough to show the type of filters according to the type of field.
...
Subscriptions are identical to queries in that way they're written, except for the keyword `query`, use `subscription`.
